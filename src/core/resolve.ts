...
function grantExp(unit: Unit, amount: number, state?: GameState): void {
  unit.exp += amount;
  syncLevelFromExp(unit);
...
export function performMove(state: GameState, unit: Unit, to: Vec2, events: GameEvent[]): boolean {
  ...
  if (unit.faction === "player") {
    ...
    const weaponIndex = state.fieldWeapons.findIndex((i) => i.x === to.x && i.y === to.y);
...
export function performAttack(
  ...
  grantExp(attacker, main.damage * VETERANCY.expPerDamage, state);
  ...
  if (defender.hp > 0 && canCounter(state, attacker, defender)) {
    ...
    grantExp(defender, counterDamage * VETERANCY.expPerDamage, state);
  ...
  if (defender.hp <= 0) {
    ...
    grantExp(attacker, VETERANCY.expPerRout, state);
  }
  if (attacker.hp <= 0) {
    routUnit(state, attacker, events);
    grantExp(defender, VETERANCY.expPerRout, state);
  }
...
export function performItem(
  ...
  if (def.targeting === "self") {
    ...
    if (canExp) {
      grantExp(unit, def.expGain ?? 0, state);
    }
